...
**2.** (4 points) Draw 100 values from a $N(2, 5)$ distribution (hint: use `rnorm()`). Create a `for` loop that goes through all the values in the sample and standardizes them. Make a histogram of the resulting distribution.

Recall: A value $z$ is standardized if:

$$z = \frac{x-\mu}{\sigma},$$
where $\mu$ and $\sigma$ are the mean and standard deviation of the (original) distribution, respectively.
