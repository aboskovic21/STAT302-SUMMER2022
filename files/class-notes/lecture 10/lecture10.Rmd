...
**Fact:** We know the sum of exponential distributions give you a gamma distribution. Specifically, if $X_1, X_2,\dots,\, X_n \sim Exp(\lambda)$, then $\sum_{i=1}^n X_i \sim Gamma(n,\lambda)$.
...
unifs <- unifs %>%
  as.data.frame() %>%
  mutate(above.75 = ifelse(m > 0.75, 1, 0))

sum(unifs$above.75 / 1000)
```
